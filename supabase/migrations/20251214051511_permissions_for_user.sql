...
-- all the actions that a user can do to another tag or role
create or replace view public.user_single_action
with(security_invoker = true)
as select distinct on (profiles_for_urole.profile_id, single_action.id)
	profiles_for_urole.profile_id,
...
with(security_invoker = true)
as select distinct on (user_single_action.profile_id, victim_profile_id, user_single_action.id)
	user_single_action.profile_id as profile_id,
	(
		case
			when user_single_action.victim_type = 'self' then user_single_action.profile_id
			when for_role.role_id is null then for_tag.profile_id
			else for_role.profile_id
		end
	) as victim_profile_id,
	user_single_action.action_type,
	user_single_action.action_data,
	user_single_action.id as id
from
	public.user_single_action
	left join public.profiles_for_urole for_role on user_single_action.victim_type = 'role' and user_single_action.victim_id = for_role.role_id
	left join public.profiles_for_utag for_tag on user_single_action.victim_type = 'tag' and user_single_action.victim_id = for_tag.utag_id
where
	for_role.role_id is not null or for_tag.utag_id is not null or user_single_action.victim_type = 'self'
;
...
for update to anon, authenticated using (
	id in (
		select
			victim_profile_id
		from
			public.user_victim_single_action
		where
