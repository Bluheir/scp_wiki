...
const common = {
	...
	password: (min: number, max: number) =>
		z
			.string()
			.min(min, { error: m.auth_invalidPassword({ min, max }) })
			.max(max, { error: m.auth_invalidPassword({ min, max }) })
}
		...
		password: common.password(8, 255).refine(
			(value) => {
				...
				for (const char of value) {
					if (!hasLower && alphabetLower.includes(char)) { hasLower = true }
					else if (!hasUpper && alphabetUpper.includes(char)) { hasUpper = true }
					else if (!hasNumber && numbers.includes(char)) { hasNumber = true }
					else if (!hasSymbol && symbols.includes(char)) { hasSymbol = true }
				}
