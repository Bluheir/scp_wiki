...
<div class="flex justify-center py-40 select-none">
	<div class="px-8 py-4 hover:shadow-2xl transition rounded-box bg-base-200 flex border border-base-content/10 w-230 gap-6">
		<form method="POST" use:enhance class="card w-sm gap-y-1.5 py-4">
			<h1 class="bold text-2xl font-bold">Welcome back Doctor [REDACTED]</h1>
				...
				<Control>
					{#snippet children({ props })}
						...
						<label class="input input-sm w-full">
							...
							<input type="email" {...props} bind:value={$formData.email} placeholder="Email" />
						</label>
					{/snippet}
				</Control>
				<Description class="text-xs">Email address for the account</Description>
			...
			<Field {form} name="password">
				...
				<Description class="text-xs">Account password</Description>
				<FieldErrors class="text-xs text-error"/>
			</Field>
